...
BEGIN
    ...
    SELECT CASE 
        ...
        THEN RAISE(ROLLBACK, 'Yetersiz stok')
    END;
...
BEGIN
    -- Fiyat null olmasın
    SELECT RAISE(ROLLBACK, 'Geçerli fiyat bulunamadı')
    WHERE NOT EXISTS (
        SELECT 1 
...
AND OLD.status IN ('ready', 'preparing') -- Sadece bu durumlarda stok iadesi
BEGIN
    -- Stokları geri ekle
    UPDATE raw_materials
...
BEGIN
    -- Status sırası kontrolü
    SELECT CASE
        WHEN OLD.status = 'new' AND NEW.status NOT IN ('confirmed', 'cancelled')
        THEN RAISE(ROLLBACK, 'Geçersiz durum değişikliği')
...
BEGIN
    -- Stok kontrolü
    SELECT RAISE(ROLLBACK, 'Yetersiz stok')
    WHERE EXISTS (
        SELECT 1 
