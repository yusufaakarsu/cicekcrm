...
var require_rewriteframes = __commonJS({
  "../../node_modules/.pnpm/@sentry+integrations@7.76.0/node_modules/@sentry/integrations/cjs/rewriteframes.js"(exports) {
    Object.defineProperty(exports, "__esModule", { value: !0 });
    var utils = require_cjs(), RewriteFrames2 = class {
      /**
       * @inheritDoc
       */
      static __initStatic() {
       ...
       */
      constructor(options = {}) {
        RewriteFrames2.prototype.__init.call(this), this.name = RewriteFrames2.id, options.root && (this._root = options.root), this._prefix = options.prefix || "app:///", options.iteratee && (this._iteratee = options.iteratee);
      }
      /**
       * @inheritDoc
       */
...
function setupSentry(request, context, dsn, clientId, clientSecret, coloMetadata, versionMetadata, accountId, scriptId) {
  ...
  let sentry = new import_toucan_js.Toucan({
    ...
    integrations: [
      new import_toucan_js.RewriteFrames({
        iteratee(frame) {
...
var AssetsManifest = class {
  ...
  async getWithBinarySearch(pathname) {
  ...
  async getWithInterpolationSearch(pathname) {
    let pathHash = await hashPath(pathname), entry = interpolationSearch(
      new Uint8Array(this.data, 20),
      pathHash
    );
    return entry ? contentHashToKey(entry) : null;
  ...
  if (arr.byteLength === 0)
    return !1;
  let low = 0, high = arr.byteLength / 40 - 1, searchValueNumber = uint8ArrayToNumber(searchValue);
  for (; low <= high; ) {
    if (low === high) {
      let current2 = new Uint8Array(
        arr.buffer,
        arr.byteOffset + low * 40,
        16
      );
      if (current2.byteLength !== searchValue.byteLength)
        throw new TypeError(
          "Search value and current value are of different lengths"
        );
      return compare(current2, searchValue) === 0 ? new Uint8Array(
        arr.buffer,
        arr.byteOffset + low * 40,
        40
      ) : !1;
    }
    let lowValue = new Uint8Array(
      arr.buffer,
      arr.byteOffset + low * 40,
      16
    ), highValue = new Uint8Array(
      arr.buffer,
      arr.byteOffset + high * 40,
      16
    ), lowValueNumber = uint8ArrayToNumber(lowValue), denominator = uint8ArrayToNumber(highValue) - lowValueNumber;
    if (denominator <= 0n)
      throw new TypeError("Search space is unordered");
    let numerator = searchValueNumber - lowValueNumber;
    if (numerator < 0n)
      return !1;
    let mid = Math.floor(
    ...
    let cmp = compare(current, searchValue);
    if (cmp === 0)
...
var src_default = class extends WorkerEntrypoint {
  ...
  async unstable_exists(pathname) {
    let analytics = new ExperimentAnalytics(this.env.EXPERIMENT_ANALYTICS), performance = new PerformanceTimer(this.env.UNSAFE_PERFORMANCE), INTERPOLATION_EXPERIMENT_SAMPLE_RATE = 0.5, searchMethod = "binary";
    Math.random() < INTERPOLATION_EXPERIMENT_SAMPLE_RATE && (searchMethod = "interpolation"), analytics.setData({ manifestReadMethod: searchMethod }), this.env.COLO_METADATA && this.env.VERSION_METADATA && this.env.CONFIG && analytics.setData({
      accountId: this.env.CONFIG.account_id,
      experimentName: "manifest-read-timing"
    });
    let startTimeMs = performance.now();
    try {
      let assetsManifest = new AssetsManifest(this.env.ASSETS_MANIFEST);
      if (searchMethod === "interpolation")
        try {
          return await assetsManifest.getWithInterpolationSearch(pathname);
        } catch {
          return analytics.setData({ manifestReadMethod: "binary-fallback" }), await assetsManifest.getWithBinarySearch(pathname);
        }
      else
